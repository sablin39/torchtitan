...
def cross_entropy_with_l2wrap_loss(
    ...
    *,
    ...
    """Cross-entropy plus RWKV's max-logit L2Wrap regularizer."""
    if isinstance(pred, DTensor) and any(
        isinstance(placement, Shard) and placement.dim in (-1, pred.ndim - 1)
        for placement in pred.placements
    ):
        raise ValueError("L2Wrap is not supported with vocab-sharded loss parallelism.")
    ...
    penalty = (
        0.5
        * l2_wrap_factor
        * (max_logits.square() * valid_tokens.to(max_logits.dtype)).sum()
    )
    return ce + penalty
...
class CrossEntropyLoss(BaseLoss):
    ...
    class Config(BaseLoss.Config):
        ...
        def __post_init__(self) -> None:
            if self.l2_wrap_factor < 0:
                raise ValueError(
                    "l2_wrap_factor must be non-negative, got " f"{self.l2_wrap_factor}"
                )

    def __init__(self, config: Config, *, compile_config: CompileConfig | None = None):
        self.l2_wrap_factor = config.l2_wrap_factor
        self.fn: LossFunction = (
            cross_entropy_with_l2wrap_loss
            if config.l2_wrap_factor > 0
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        if self.l2_wrap_factor > 0:
            loss_kwargs["l2_wrap_factor"] = self.l2_wrap_factor
        loss = self.fn(pred, labels, **loss_kwargs)
        if global_valid_tokens is not None:
            # TODO(pianpwk): Teach spmd_types that P / scalar preserves P.
            is_type_checking = spmd.is_type_checking()
            with spmd.no_typecheck():
