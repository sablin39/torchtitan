...
def register_moe_load_balancing_hook(
    ...
    def _iter_moe_layers(
        ...
        for model_part in model_parts:
            ...
            for layers_fqn in ("layers", "llm.layers"):
                try:
                    candidate = model_part.get_submodule(layers_fqn)
                except AttributeError:
                    continue
                if isinstance(candidate, nn.ModuleDict):
                    layers = candidate
                    break
            if layers is None:
                continue
    ...
    def _tokens_per_expert(moe: _MoELike) -> torch.Tensor:
        tensor = getattr(moe, "tokens_per_expert_E", None)
        if tensor is None:
            tensor = getattr(moe, "tokens_per_expert", None)
        if tensor is None:
            raise ValueError("MoE layer does not expose tokens-per-expert counters")
        return tensor

    def _expert_bias(moe: _MoELike) -> torch.Tensor:
        tensor = getattr(moe, "expert_bias_E", None)
        if tensor is None:
            tensor = getattr(moe, "expert_bias", None)
        if tensor is None:
            raise ValueError("MoE layer does not expose an expert-bias tensor")
        return tensor
    ...
    def _update_expert_bias(
        ...
        for transformer_block, moe in _iter_moe_layers(model_parts):
            tokens_per_expert_E = _tokens_per_expert(moe)
        ...
        with torch.no_grad():
            for _transformer_block, moe in _iter_moe_layers(model_parts):
                ...
                _expert_bias(moe).add_(expert_bias_delta_E)
                _tokens_per_expert(moe).zero_()
