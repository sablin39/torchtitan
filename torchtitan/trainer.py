...
class Trainer(torch.distributed.checkpoint.stateful.Stateful, Configurable):
    ...
    def post_dataloading_process(
        ...
        if uses_fla_cp and (
            "pixel_values" in extra_kwargs or "pixel_values_videos" in extra_kwargs
        ):
    ...
    def train_step(
        ...
        sync_exhaustion = getattr(self, "_sync_dataloader_exhaustion", None)
        any_exhausted = (
            sync_exhaustion(dataloader_exhausted)
            if callable(sync_exhaustion)
            else dataloader_exhausted
        )
        if any_exhausted:
        ...
        if hasattr(self, "n_valid_tokens_seen"):
            extra_metrics["n_valid_tokens_seen"] = self.n_valid_tokens_seen
            extra_metrics["n_nonpad_tokens_seen"] = self.n_nonpad_tokens_seen
        consume_data_metrics = getattr(self, "_consume_data_metrics", None)
        if callable(consume_data_metrics):
            extra_metrics.update(consume_data_metrics())
        collect_moe_metrics = getattr(self, "_collect_moe_metrics", None)
        if callable(collect_moe_metrics):
            extra_metrics.update(collect_moe_metrics())
