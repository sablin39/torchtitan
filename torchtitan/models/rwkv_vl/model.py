...
class VisualAdapter(Module):
    ...
    def __init__(self, config: Config):
        super().__init__()
        self.config = config
        self.pre_norm = LayerNorm(config.encoder_dim, eps=config.norm_eps)
        self.mlp = Sequential(
            _linear(config.encoder_dim, config.hidden_dim, bias=True),
            ReLU(),
            _linear(config.hidden_dim, config.project_dim, bias=True),
        )
...
class RWKV7VLForConditionalGeneration(BaseModel):
    ...
    def _get_vision_embeds(
        ...
        *,
        ...
        merged_embeds, _ = self.vision_encoder(pixel_values, grid_thw=grid_thw)
        merged_embeds = self.proj(merged_embeds)
    ...
    def _prepare_inputs_embeds(
        ...
        *,
        ...
        if pixel_values is not None and grid_thw is not None:
            merged_embeds, num_tokens_per_item = self._get_vision_embeds(
                pixel_values,
                grid_thw=grid_thw,
            )
