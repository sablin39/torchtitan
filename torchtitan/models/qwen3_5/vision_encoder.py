...
class Qwen35VisionEncoder(Module):
    ...
    def forward(
        ...
        *,
        ...
        """
        return self._forward_with_layer_callback(
            pixel_values,
            grid_thw=grid_thw,
        )

    def _forward_with_layer_callback(
        self,
        pixel_values: torch.Tensor,
        *,
        grid_thw: torch.Tensor,
        layer_callback: Callable[[int, torch.Tensor], None] | None = None,
    ) -> torch.Tensor:
        """Run the packed encoder and optionally expose intermediate layers."""
        ...
        for layer_index, layer in self.layers.items():
            ...
            if layer_callback is not None:
                layer_callback(int(layer_index), x)
