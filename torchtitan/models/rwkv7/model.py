...
def get_rwkv7_moe_nparams_and_flops(
    ...
    *,
    ...
    return nparams, 6 * nparams_active
...
class RWKV7ChannelMixExperts(Module):
    ...
    def forward(
        ...
        token_indices = (
            token_indices_experts_sorted // selected_experts_indices.shape[1]
        )
...
class RWKV7MoEChannelMix(Module):
    ...
    def __init__(self, config: Config):
        ...
        if config.hidden_act != "sqrelu":
            raise ValueError(
                "RWKV7MoEChannelMix currently supports hidden_act='sqrelu'"
            )
    ...
    class Config(BaseModel.Config):
        ...
        def update_from_config(self, *, trainer_config, **kwargs) -> None:
            ...
            if parallelism.expert_parallel_degree > 1:
                raise NotImplementedError(
                    "RWKV7 MoE ChannelMix currently uses local experts and does not "
        ...
        def get_nparams_and_flops(self, model: Module, seq_len: int) -> tuple[int, int]:
            if self.llm.moe_channel_mix_start_layer is not None:
                if self.llm.moe_channel_mix_num_experts is None:
                    raise ValueError(
                        "moe_channel_mix_num_experts is required when MoE is enabled"
                    )
