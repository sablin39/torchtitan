...
def test_post_dataloading_process_counts_unmasked_tokens():
    trainer = cast(
        Trainer,
        SimpleNamespace(
            model_config=SimpleNamespace(uses_fla_context_parallel=False),
            model_parts=[],
            parallel_dims=SimpleNamespace(cp_enabled=False),
            config=SimpleNamespace(parallelism=SimpleNamespace(spmd_backend="dtensor")),
            device=torch.device("cpu"),
            ntokens_seen=0,
            n_valid_tokens_seen=0,
            n_nonpad_tokens_seen=0,
        ),
    )
    labels = torch.tensor([[1, 2, 3]])

    inputs, processed_labels, extra_kwargs = Trainer.post_dataloading_process(
        trainer,
        {"input": torch.tensor([[4, 5, 6]])},
        labels,
    )

    torch.testing.assert_close(inputs, torch.tensor([[4, 5, 6]]))
    assert processed_labels is labels
    assert extra_kwargs == {}
    assert trainer.ntokens_seen == 3
    assert trainer.n_valid_tokens_seen == 3
    assert trainer.n_nonpad_tokens_seen == 3
