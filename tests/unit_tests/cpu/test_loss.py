...
class TestLoss(unittest.TestCase):
    def test_cross_entropy_l2wrap_matches_reference(self):
        logits = torch.tensor(
        ...
        factor = 0.2
        loss_fn = CrossEntropyLoss(CrossEntropyLoss.Config(l2_wrap_factor=factor))

        loss, metrics = loss_fn(
            logits,
            labels,
            global_valid_tokens=torch.tensor(1.0),
        )
        expected = (
            F.cross_entropy(
                logits.float(),
            ...
            + 0.5 * factor * logits[0].max().square()
        )
    ...
    def test_cross_entropy_rejects_negative_l2wrap(self):
        with self.assertRaisesRegex(ValueError, "must be non-negative"):
            CrossEntropyLoss.Config(l2_wrap_factor=-1e-4)
